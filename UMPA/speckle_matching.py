...
def match(Isample, Iref, Nw, mask=None, step=1, max_shift=4, df=True):
    ...
    if any(not x.flags.c_contiguous for x in Isample):
        print(
            "Warning: provided list of sample frames are not c contiguous - working with a copy."
        )
        Isample = [x.copy() for x in Isample]

    if any(not x.flags.c_contiguous for x in Iref):
        print(
            "Warning: provided list of reference frames are not c contiguous - working with a copy."
        )
        Iref = [x.copy() for x in Iref]

    if df:
        PM = model.UMPAModelNoDF(
            sam_list=Isample, ref_list=Iref, mask_list=mask, window_size=Nw
        )
    else:
        PM = model.UMPAModelDF(
            sam_list=Isample, ref_list=Iref, mask_list=mask, window_size=Nw
        )
...
def match_unbiased(
    Isample, Iref, Nw, mask=None, step=1, max_shift=4, df=True, bias=True
):
    """
    ...
    if bias is True:
        ...
        if df:
            PMref = model.UMPAModelDF(
                sam_list=Iref, ref_list=Iref, mask_list=mask, window_size=Nw
            )
        else:
            PMref = model.UMPAModelNoDF(
                sam_list=Iref, ref_list=Iref, mask_list=mask, window_size=Nw
            )
...
def test_gaussian_abc(Nw=2, step=10, max_shift=4):
    ...
    s = u.prep_simul()
    PM = model.UMPAModelDFKernel(
        sam_list=s["meas"], ref_list=s["ref"], mask_list=None, window_size=Nw
    )
    sh = PM.sh
    abc = np.zeros(sh + (3,))
    abc[:, :, 0] = 0.1
...
def test(Nw=1, step=1, max_shift=4):
    ...
    print(
        '%s: "match" (single precision) completed in %f seconds'
    ...
    sh = s["ref"][0].shape
    np.random.seed(15)
    mask = (np.random.uniform(size=sh) < 0.95).astype(float)
    ...
    print(
        '%s: "match" with mask completed in %f seconds' % (__name__, time.time() - t0)
    )

    t0 = time.time()
    r4 = match_unbiased(
        s["meas"], s["ref"], Nw, mask=mask_list, step=step, max_shift=max_shift
    )
    print(
        '%s: "match_unbiased" with mask completed in %f seconds'
        % (__name__, time.time() - t0)
    )
    ...
    print(
        '%s: "match" (float) with mask completed in %f seconds'
        % (__name__, time.time() - t0)
    )

    t0 = time.time()
    r6 = match_unbiased(
        smeas_f, sref_f, Nw, mask=mask_f, step=step, max_shift=max_shift
    )
    print(
        '%s: "match_unbiased" (float) with mask completed in %f seconds'
        % (__name__, time.time() - t0)
    )
    ...
    print(
        '%s: "model_map" with mask completed in %f seconds'
        % (__name__, time.time() - t0)
    )
