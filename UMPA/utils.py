...
def quad_fit(a):
    ...
    p = r[0]
    x0 = -(
        np.matrix([[2 * p[3], p[5]], [p[5], 2 * p[4]]]).I * np.matrix([p[1], p[2]]).T
    ).A1
    c = p[0] + 0.5 * (p[1] * x0[0] + p[2] * x0[1])
    h = np.matrix([[p[3], 0.5 * p[5]], [0.5 * p[5], p[4]]])
    return c, x0, h
...
def pshift(a, ctr):
    ...
    for comb_i in range(comb_num):
        ...
        # input array "a".
        comb = np.asarray(
            tuple(("{0:0" + str(a.ndim) + "b}").format(comb_i)), dtype=int
        )

        # fdm: get the weighted contribution for the shift corresponding to this combination
        cc = ctri + comb
        out += (
            np.roll(a, shift=-cc, axis=range(a.ndim)) * ctrx[comb, range(a.ndim)].prod()
        )
    return out
...
def sub_pix_min_quad(a, width=1):
    ...
    mindist, r = quad_max(
        -np.real(
            a[
                (cmin[0] - width) : (cmin[0] + width + 1),
                (cmin[1] - width) : (cmin[1] + width + 1),
            ]
        )
    )
    r -= width - cmin
...
M = np.array(
    [
        np.multiply.outer(coeffs[j], coeffs[i]).ravel()
        for i in range(4)
        for j in range(4)
    ]
).T
...
def allf(x0, c):
    ...
    return np.dot(
        np.array(
            [
                np.multiply.outer(y**e0, x**e0).ravel(),
                ...
                np.multiply.outer(e0 * e1 * y**e2, x**e0).ravel(),
            ]
        ),
        c,
    )
...
def sub_pix_cc_linear(a, x0):
    ...
    *** subpixel position is set in-place in x0! ***
    ...
    for i in range(30):
        ...
        dx = -np.array([fyy * fx - fxy * fy, -fxy * fx + fxx * fy]) / (
            fxx * fyy - fxy * fxy
        )
        dx.clip(-0.5, 0.5, out=dx)
        x0 += dx
        if (dx * dx).sum() < tol:
            break
...
def free_nf(w, l, z, pixsize=1.0):
    ...
    if min(sh) / np.sqrt(2.0) < z * l:
        print(
            "Warning: z > N/(sqrt(2)*lamda) = %.6g: this calculation could fail."
            % (min(sh) / (l * np.sqrt(2.0)))
        )
    ...
    q2 = np.sum(
        (
            ...
            * np.array([1.0 / sh[0], 1.0 / sh[1]]).reshape((2, 1, 1))
        )
        ** 2,
        axis=0,
    )
...
def prep_simul(
    ...
    ssize=2e-6,
    speckles=True,
    dn=7.79552408e-07 - 1j * 1.2177146e-09,
    z=5e-2,
    noise=None,
    pyr_width=300e-6,
    ...
    if obj == "sphere":
        ...
        feature = (
            ...
            * psize
            * np.real(
                scimath.sqrt((cyl_radius / psize) ** 2 - (yy - shape[0] / 2.0) ** 2)
            )
        )
    elif obj == "cyl_x":
        feature = (
            ...
            * psize
            * np.real(
                scimath.sqrt((cyl_radius / psize) ** 2 - (xx - shape[1] / 2.0) ** 2)
            )
        )
    elif obj == "pyramid":
        pyr_height = pyr_width / 2.0
        # "1-norm" / Manhattan norm:
        dist1 = (
            np.max(
                np.array([np.abs(xx - shape[1] / 2.0), np.abs(yy - shape[0] / 2.0)]), 0
            )
            * psize
        )
        ...
        feature = (
            wedge_height / 2.0 + wedge_height / wedge_width * (xx - np.mean(xx)) * psize
        )
        dist1 = (
            np.max(
                np.array([np.abs(xx - shape[1] / 2.0), np.abs(yy - shape[0] / 2.0)]), 0
        ...
        feature[dist1 > wedge_width / 2.0] = 0
    elif obj == "wedge_y":
        wedge_height = wedge_width
        feature = (
            wedge_height / 2.0 + wedge_height / wedge_width * (yy - np.mean(yy)) * psize
        )
        dist1 = (
            np.max(
                np.array([np.abs(xx - shape[1] / 2.0), np.abs(yy - shape[0] / 2.0)]), 0
        ...
        feature = (
            np.pad(logo, ((shape[0] + 50, shape[0]), (shape[1] + 50, shape[1])))
            * logo_height
        )
    else:
    ...
    if profile == "flat":
        ...
        wf_flat = np.exp(
            -0.5
            * (((xx - np.mean(xx)) / sigma1) ** 2 + ((yy - np.mean(yy)) / sigma0) ** 2)
        )
    ...
    if step_random:
        ...
        if type(steps) is np.ndarray:
            ...
            if np.isclose(iroot, root):
                ...
                pos = (
                    step_size
                    * np.indices((iroot + 1, iroot + 1)).reshape((2, -1)).T[:steps]
                )
...
def plot_cost(
    ...
    c, t, d = get_cost(
        model, i + window_size + max_shift, j + window_size + max_shift, N=max_shift - 1
    )

    if do_plot:
        plt.figure(figsize=figsize)
        plt.subplot(131)
        plt.title("dx, dot:(i,j)=(%d,%d)" % (i, j))
        plt.imshow(fit["dx"], vmin=vmin, vmax=vmax)
        plt.plot(j, i, "ro")
        plt.subplot(132)
        plt.title("dy, dot:(i,j)=(%d,%d)" % (i, j))
        plt.imshow(fit["dy"], vmin=vmin, vmax=vmax)
        plt.plot(j, i, "ro")
        plt.subplot(133)
        plt.title(
            "Cost function at (i,j) = (%d,%d)\ndx=%.2f, dy=%.2f, f=%.3g, N_calls=%d"
            % (
                i,
                j,
                fit["dx"][i, j],
                fit["dy"][i, j],
                fit["f"][i, j],
                fit["debug_Ncalls"][i, j],
            )
        )
        imsh_cost(c)
        plt.plot(fit["dx"][i, j], fit["dy"][i, j], "ro")
        plt.tight_layout()
    return c, t, d
